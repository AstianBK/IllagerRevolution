...
public class FallenKnightEntity extends ReanimatedEntity implements GeoEntity, IHasInventory {
    ...
    public void aiStep() {
        ...
        if (this.dispawnTimer==0 && this.isEndless) {
            this.removeEntityOfList();
            this.die(this.damageSources().fall());
        }
        if(this.isAttacking()){
            this.attackTimer--;
        }
    ...
    private <E extends GeoEntity> PlayState predicate(AnimationState<E> event) {
        if(this.isArmed()){
            if (event.isMoving() && !this.isAttacking() && !this.isAggressive()) {
                event.getController().setAnimation(RawAnimation.begin().then("animation.fallenknight.walk1", Animation.LoopType.LOOP));
            }else if(event.isMoving() && this.isAggressive() && !this.isAttacking()){
                event.getController().setAnimation(RawAnimation.begin().then("animation.fallenknight.walk2",Animation.LoopType.LOOP));
            } else if (this.isAttacking()){
                event.getController().setAnimation(RawAnimation.begin().then("animation.fallenknight.attack1",Animation.LoopType.PLAY_ONCE));
            }else   {
                event.getController().setAnimation(RawAnimation.begin().then("animation.fallenknight.idle",Animation.LoopType.LOOP));
            }
        }else{
            if(this.isUnarmed() && !this.isOnGroundUnarmed()){
                event.getController().setAnimation(RawAnimation.begin().then("animation.fallenknight.death1",Animation.LoopType.HOLD_ON_LAST_FRAME));
            } else if (this.isRearmed() && !this.isOnGroundUnarmed()) {
                event.getController().setAnimation(RawAnimation.begin().then("animation.fallenknight.revive1",Animation.LoopType.HOLD_ON_LAST_FRAME));
            }else {
                event.getController().setAnimation(RawAnimation.begin().then("animation.fallenknight.death2",Animation.LoopType.LOOP));
            }
        }
