...
public class SoulEaterEntity extends Monster implements GeoEntity {
            ...
            if(this.drainDuration>0){
                if(this.checkIsAlive(target)){
                    if(this.tickCount%20==0){
                        ...
                        if(target.hurt(damageSources().magic(),1.0F)){
                            this.heal(1.0F);
                        }
                    }
                }else {
    ...
    private   <E extends GeoEntity> PlayState predicateMaster(AnimationState<E> event) {
        event.getController().setAnimation(RawAnimation.begin().then("animation.souleater.idle", Animation.LoopType.LOOP));
    ...
    private   <E extends GeoEntity> PlayState predicateHead(AnimationState<E> event) {
        if(this.isDrainSoul() && !this.swinging && !event.isMoving()){
            event.getController().setAnimation(RawAnimation.begin().then("animation.souleater.drain",Animation.LoopType.LOOP));
    ...
    protected void populateDefaultEquipmentSlots(DifficultyInstance p_219136_) {
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar data) {
        data.add(new AnimationController<>(this,"predicate_master",
                0,this::predicateMaster));
        data.add(new AnimationController<>(this,"predicate_head",
