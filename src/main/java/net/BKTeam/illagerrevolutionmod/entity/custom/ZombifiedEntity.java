    ...
    private <E extends GeoEntity> PlayState predicate(AnimationState<E> event) {
        if(!this.getIsSpawned()){
            if (event.isMoving() && !this.isAttacking()) {
                event.getController().setAnimation(RawAnimation.begin().then("zombified.illager.walk", Animation.LoopType.LOOP));
            }else if(this.isAttacking()){
                event.getController().setAnimation(RawAnimation.begin().then("zombified.illager.attack", Animation.LoopType.PLAY_ONCE));
            }else {
                event.getController().setAnimation(RawAnimation.begin().then("zombified.illager.idle", Animation.LoopType.LOOP));
            }
        }else {
            event.getController().setAnimation(RawAnimation.begin().then("zombified.illager.revive", Animation.LoopType.HOLD_ON_LAST_FRAME));
        }
