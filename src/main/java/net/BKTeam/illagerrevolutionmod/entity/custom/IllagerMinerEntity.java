...
public class IllagerMinerEntity extends Monster implements GeoEntity, InventoryCarrier {
    ...
    private   <E extends GeoEntity> PlayState predicate(AnimationState<E> event) {
        ...
        if (event.isMoving() && !this.isAggressive() && !this.isAttacking()) {
            event.getController().setAnimation(RawAnimation.begin().then("animation.illagerminer.walk"+(this.isHasItems() ? "3" : ""), Animation.LoopType.LOOP));

        }else if (this.isAggressive() && event.isMoving() && !this.isAttacking()){
            event.getController().setAnimation(RawAnimation.begin().then("animation.illagerminer.walk2",Animation.LoopType.LOOP));

        }else if (this.isAttacking()){
            event.getController().setAnimation(RawAnimation.begin().then("animation.illagerminer.attack",Animation.LoopType.PLAY_ONCE));

        }else event.getController().setAnimation(RawAnimation.begin().then("animation.illagerminer.idle1",Animation.LoopType.LOOP));
