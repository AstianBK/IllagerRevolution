...
public class IllagerBeastTamerEntity extends SpellcasterKnight implements GeoEntity, InventoryCarrier, RangedAttackMob {
    ...
    private   <E extends GeoEntity> PlayState predicate(AnimationState<E> event) {
        if (event.isMoving() && !this.isAggressive() && !this.isCastingSpell() && !this.isPassenger()) {
            event.getController().setAnimation(RawAnimation.begin().then("animation.beasttamerillager.walk", Animation.LoopType.LOOP));
        }
        else if (this.isWieldingTwoHandedWeapon() && event.isMoving() && !this.isPassenger() && !this.isCastingSpell()){
            event.getController().setAnimation(RawAnimation.begin().then("animation.beasttamerillager.walk2",Animation.LoopType.LOOP));
        }else if (this.isWieldingTwoHandedWeapon() && !event.isMoving() && this.isAggressive() && !this.isCastingSpell() && !this.isPassenger()){
            event.getController().setAnimation(RawAnimation.begin().then("animation.beasttamerillager.attack1",Animation.LoopType.LOOP));
        }else if (this.isCastingSpell()){
            event.getController().setAnimation(RawAnimation.begin().then("animation.beasttamerillager."+(this.isPassenger() ? "sit2" : "summon"),Animation.LoopType.LOOP));
        }
        else event.getController().setAnimation(RawAnimation.begin().then(this.isPassenger()?"animation.beasttamerillager.sit" : "animation.beasttamerillager.idle",Animation.LoopType.LOOP));
