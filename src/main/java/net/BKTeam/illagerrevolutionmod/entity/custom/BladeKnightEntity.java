...
public class BladeKnightEntity extends SpellcasterKnight implements GeoEntity, InventoryCarrier {
    ...
    private <E extends GeoEntity> PlayState predicate(AnimationState<E> event) {
        if (event.isMoving() && !this.isAggressive() && !this.isCastingSpell() && (!this.hasCombo() || !this.continueAnim)) {
            event.getController().setAnimation(RawAnimation.begin().then("animation.IllagerBladeKnight.walk1", Animation.LoopType.LOOP));
        } else if (event.isMoving() && this.isAggressive() && (!this.hasCombo() || !this.continueAnim)) {
            event.getController().setAnimation(RawAnimation.begin().then("animation.IllagerBladeKnight.walkhostile1",Animation.LoopType.LOOP));
        } else if (this.hasCombo() && this.continueAnim && this.getCombo()!=Combo.EXHAUSTED) {
            event.getController().setAnimation(RawAnimation.begin().then(this.getCombo().location + (this.getCombo() == Combo.COMBO_SPIN ? this.getComboState().nameAttack1 : this.getComboState().nameAttack2),Animation.LoopType.PLAY_ONCE));
        } else if (this.hasCombo() && this.continueAnim && this.getCombo()==Combo.EXHAUSTED) {
            event.getController().setAnimation(RawAnimation.begin().then(this.getCombo().location ,Animation.LoopType.LOOP));
        } else if (this.isCastingSpell()) {
            event.getController().setAnimation(RawAnimation.begin().then("animation.IllagerBladeKnight.summon1",Animation.LoopType.LOOP));
        } else
            event.getController().setAnimation(RawAnimation.begin().then("animation.IllagerBladeKnight.idle1",Animation.LoopType.LOOP));
    ...
    class BKSummonHunterSpellGoal extends SpellcasterUseSpellGoal {
        ...
        protected void performSpellCasting() {
            ...
            Level souce = BladeKnightEntity.this.level();
                ...
                if(this.goalOwner.getCombo()==Combo.COMBO_SPIN){
                    ...
                    if(this.goalOwner.getComboState()==ComboState.THIRD_HIT){
                        ...
                        if(this.goalOwner.animationTimer==8 ){
                            ...
                            BlockPos pos = new BlockPos((int) this.goalOwner.getX(), (int) (this.goalOwner.getY()+1.5d), (int) this.goalOwner.getZ());
                            ...
                            for(Entity living : targets){
                                ...
                                if(living instanceof LivingEntity){
                                    if (entityHitDistance <= 7 - 0.3 ) {
                                        living.hurt(this.goalOwner.damageSources().mobAttack(this.goalOwner), 1.0F+this.goalOwner.getTotalDamage()*0.4F);
                                        sound=true;
                                    }
                            ...
                            for(Entity living : targets){
                                ...
                                if(living instanceof  LivingEntity){
                                    if (entityHitDistance <= 7 - 0.3 && (entityRelativeAngle <= arc / 2 && entityRelativeAngle >= -arc / 2) || (entityRelativeAngle >= 360 - arc / 2 || entityRelativeAngle <= -360 + arc / 2) ) {
                                        living.hurt(this.goalOwner.damageSources().mobAttack(this.goalOwner), 1.0F+this.goalOwner.getTotalDamage()*0.25F);
                                    }
                                }else if(living instanceof Projectile projectile && projectile.getOwner()!=this.goalOwner){
                                    projectile.shoot(projectile.getX()-this.goalOwner.getX(),projectile.getY()-this.goalOwner.getY(),projectile.getZ()-this.goalOwner.getZ(),1f,0.1f);
                                }
                                ...
                                if(this.goalOwner.animationTimer==5){
                                    ...
                                    for(LivingEntity living : targets){
                                        ...
                                        if (entityHitDistance <= 5 - 0.3 && (entityRelativeAngle <= arc / 2 && entityRelativeAngle >= -arc / 2) || (entityRelativeAngle >= 360 - arc / 2 || entityRelativeAngle <= -360 + arc / 2) ) {
                                            ...
                                            if(living.hurt(this.goalOwner.damageSources().mobAttack(this.goalOwner), 1.0F+this.goalOwner.getTotalDamage()*0.5F)){
