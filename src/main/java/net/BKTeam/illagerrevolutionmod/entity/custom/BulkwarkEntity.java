...
public class BulkwarkEntity extends KnightEntity implements IAnimatable {
    ...
    private   <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event) {
        ...
        if (event.isMoving() && !this.isCharged() && !this.isStunned()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.bulkwark.walk"+s1, ILoopType.EDefaultLoopTypes.LOOP));
        ...
        else event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.bulkwark.idle"+s1, ILoopType.EDefaultLoopTypes.LOOP));
