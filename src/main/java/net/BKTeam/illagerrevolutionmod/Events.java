...
public class Events {
        ...
        if (event != null && event.getEntity() != null) {
            ...
            if(attacker instanceof Player player){
                if (bulletEntity!=null){
                    ...
                    if(player.getMainHandItem().getItem() instanceof AxeItem){
                        event.setCanceled(true);
                        entity.hurt(player.damageSources().generic(), event.getAmount()+2.0f);
                    }
            ...
            if(event.getEntity().hasEffect(InitEffect.SOUL_BURN.get())){
                ...
                if(event.getSource().is(DamageTypeTags.IS_FIRE)){
                    if (level < 3) {
                        event.getEntity().hurt(event.getEntity().damageSources().generic(),damage);
                    }else {
    ...
    public static void effectFullAmorPillager(Player player,LivingEntity living,Arrow arrow,float amount){
        living.hurt(player.damageSources().arrow(arrow,null),amount+4.0f);
        arrow.discard();
    }
