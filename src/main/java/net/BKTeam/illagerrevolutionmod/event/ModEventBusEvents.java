...
public class ModEventBusEvents {
    ...
    public static void registerParticleFactories(RegisterParticleProvidersEvent event) {
        if(SMOKE_BK_PARTICLES.isPresent()){
            event.registerSpriteSet(SMOKE_BK_PARTICLES.get(), BkSmokeParticles.Factory::new);
        }
        if(HEART_BK_PARTICLES.isPresent()){
            event.registerSpriteSet(HEART_BK_PARTICLES.get(), BkHeartParticles.Factory::new);
        }
        if (BKSOULS_PARTICLES.isPresent()){
            event.registerSpriteSet(BKSOULS_PARTICLES.get(), BKSoulsParticles.Factory::new);
        }
        if(SOUL_PROJECTILE_PARTICLES.isPresent()){
            event.registerSpriteSet(SOUL_PROJECTILE_PARTICLES.get(), SoulProjectilePParticles.Factory::new);
        }
        if(SOUL_FLAME.isPresent()){
            event.registerSpriteSet(SOUL_FLAME.get(), BKFireSoulParticles.Factory::new);
        }
        if (RUNE_CURSED_PARTICLES.isPresent()){
            event.registerSpriteSet(RUNE_CURSED_PARTICLES.get(), RuneCursedParticles.Factory::new);
        }
        if(RUNE_SOUL_PARTICLES.isPresent()){
            event.registerSpriteSet(RUNE_SOUL_PARTICLES.get(), RuneSoulParticles.Factory::new);
        }
        if(BLOOD_PARTICLES.isPresent()){
            event.registerSpriteSet(BLOOD_PARTICLES.get(), BloodBKParticles.Factory::new);
        }
    }
